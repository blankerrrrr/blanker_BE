...
본 문서는 FastAPI 기반 백엔드의 패키지 구조를 정의한다. 대상 범위는 인증, 관심사, 보관함, 관심 정보, API, DB, AI 분석이다.
...
| 비즈니스 로직은 Service에 배치 | 인증, 분석, 보관함 저장, 관심 정보 저장은 service 계층에서 처리한다. |
| DB 접근은 Repository로 격리 | SQLAlchemy 쿼리는 repository 계층에 둔다. |
| DTO와 ORM 분리 | Pydantic schema와 SQLAlchemy model을 직접 섞지 않는다. |
| AI 분석은 독립 모듈화 | Anthropic API 호출, 프롬프트, 후처리를 `ai` 패키지에 격리한다. |
...
backend/
  app/
    ...
    api/
      ...
      screenshot_analysis_requests.py
      blocked_items.py
      interest_items.py
    core/
      __init__.py
    ...
    services/
      ...
      token_service.py
      interest_service.py
      interest_catalog_import_service.py
...
| `api/interests.py` | `/api/interests` | `InterestService`, `InterestTargetService` |
...
| `api/analysis_requests.py` | `/api/analyses` | `AnalysisService` |
| `api/screenshot_analysis_requests.py` | `/api/analyses/screenshot` | `ScreenshotAnalysisService` |
...
| `models/interest.py` | `interest_catalog`, `interest_genres`, `interest_genre_mappings`, `interests` |
...
| `models/interest_item.py` | `interest_items` |
...
| `InterestRepository` | `findAll`, `findTypes`, `findGenresByType`, `save`, `saveCatalog`, `saveGenre` |
...
| `InterestItemRepository` | `countItems`, `findItems`, `findUrls`, `getItemById`, `getItemBySourceUrl`, `saveItem` |
...
| `InterestService` | 관심사 목록/종류/장르 조회, 관심사 선택 |
| `InterestCatalogImportService` | 외부 관심사와 기본 관심사 종류 적재 |
...
| `ScreenshotAnalysisService` | 스크린샷 이미지 OCR 분석 |
| `BlockedItemService` | 보관함 조회/저장/삭제 |
| `InterestItemService` | 관심 정보 조회/저장, 원본 URL 목록 조회 |

## 9. AI 분석 패키지 설계

| 파일 | 책임 |
| --- | --- |
| `ai/client.py` | Anthropic API client 래핑 |

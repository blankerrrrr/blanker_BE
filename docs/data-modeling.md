...
| 항목 | 규칙 |
| --- | --- |
| ID | 외부 노출 ID는 문자열 Public ID를 사용한다. 예: `user_`, `target_`, `analysis_` |
...
| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | varchar(40) | PK | 사용자 Public ID. 예: `user_01HZX...` |
| `email` | varchar(255) | UNIQUE, NOT NULL | 로그인 이메일 |
| `passwordHash` | varchar(255) | NOT NULL | 해시된 비밀번호 |
| `termsAgreed` | boolean | NOT NULL | 필수 약관 동의 여부 |
| `privacyAgreed` | boolean | NOT NULL | 개인정보 처리 동의 여부 |
| `createdAt` | timestamptz | NOT NULL | 가입 시각 |
| `updatedAt` | timestamptz | NOT NULL | 수정 시각 |

### 4.2 `interest_targets`

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | varchar(40) | PK | 관심 대상 Public ID. 예: `target_01HZX...` |
| `userId` | varchar(40) | FK, NOT NULL | `users.id` |
...
| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | varchar(40) | PK | 차단 설정 Public ID |
| `userId` | varchar(40) | FK, NOT NULL | `users.id` |
...
### 4.4 `analysis_requests`

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | varchar(40) | PK | 분석 요청 Public ID. 예: `analysis_01HZX...` |
| `userId` | varchar(40) | FK, NOT NULL | `users.id` |
...
### 4.5 `analysis_contents`

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | varchar(40) | PK | 분석 콘텐츠 Public ID |
| `analysisRequestId` | varchar(40) | FK, NOT NULL | `analysis_requests.id` |
...
| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | varchar(40) | PK | 분석 결과 Public ID |
| `analysisContentId` | varchar(40) | FK, UNIQUE, NOT NULL | `analysis_contents.id` |
...
| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | varchar(40) | PK | 보관함 항목 Public ID. 예: `blocked_01HZX...` |
| `userId` | varchar(40) | FK, NOT NULL | `users.id` |
| `analysisResultId` | varchar(40) | FK, NULL | 저장 근거가 된 분석 결과 |
...
### 4.8 `interest_item_groups`

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | varchar(40) | PK | 관심 정보 그룹 Public ID. 예: `interest_group_01HZX...` |
| `userId` | varchar(40) | FK, NOT NULL | `users.id` |
| `representativeItemId` | varchar(40) | FK, NULL | 대표 관심 정보. `interest_items.id` |
...
| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | varchar(40) | PK | 관심 정보 Public ID. 예: `interest_item_01HZX...` |
| `userId` | varchar(40) | FK, NOT NULL | `users.id` |
| `groupId` | varchar(40) | FK, NOT NULL | `interest_item_groups.id` |
...
| `analysis_results` - `blocked_items` | 1:N | 하나의 분석 결과가 저장 근거가 될 수 있다. |
